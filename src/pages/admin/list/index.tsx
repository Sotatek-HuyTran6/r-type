...
import { getAdminList } from 'redux/sub-admin/admin-list/actions';
import _ from 'lodash';

function AdminList() {
  const dispatch = useDispatch() as AppDispatch;
  const adminList = useSelector((state: RootState) => state.adminList);
  const { data, error, loading, total, paramsData } = adminList;
  ...
  const pagination = {
    ...
    onChange: (current: number, size: number) => {
      dispatch(setParamAdminList({ ...paramsData, page: current, size: size }));
    },
  };

  const onSearchChange = (value: string) => {
    dispatch(setParamAdminList({ ...paramsData, email: value }));
  };
  ...
  const handleBanUser = async (userId: string | number) => {
    dispatch(editUser(userId)).then((status) => {
      if (status) {
        ...
        dispatch(setDataAdminList([...cloneData]));
      }
    });
  };
  ...
  useEffect(() => {
    dispatch(getAdminList(paramsData));
  }, [dispatch, paramsData]);
